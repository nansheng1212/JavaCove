<template>
    <div class="app-container">
        ...
        <!-- 发布或修改对话框 -->
        <el-dialog class="talk-dialog" title="发布文章" v-model="addOrUpdate" append-to-body>
...
import { ElMessage, FormInstance, FormRules, UploadRawFile, UploadRequestOptions } from 'element-plus';
...
const handleFileUpload = (options: UploadRequestOptions) => {
  const file = options.file;
  const reader = new FileReader();
  reader.onload = () => {
    let markdownContent = reader.result as string;
    //设置正文
    articleForm.value.articleContent = markdownContent
    //设置标题
    articleForm.value.articleTitle = removeSpecialChars(markdownContent.split('\n')[0]);
  };
  reader.readAsText(file);
};
const exportToMarkdown = () => {
  const markdownContent = `# ${articleForm.value.articleTitle}\n\n${articleForm.value.articleContent}`;
  const blob = new Blob([markdownContent], { type: 'text/markdown' });
  const downloadLink = document.createElement('a');
  downloadLink.href = URL.createObjectURL(blob);
  downloadLink.download = `${articleForm.value.articleTitle}.md`;
